...
namespace SimChartMedicalOffice.Common.Extensions
{
    public static class LinqExtensions
    {
        ...
        public static IQueryable<T> Where<T>(this IQueryable<T> query,
            string column, object value, AppEnum.WhereOperation operation)
        {
            ...
            {
                //equal ==
                case AppEnum.WhereOperation.Equal:
                    condition = Expression.Equal(memberAccess, filter);
                    lambda = Expression.Lambda(condition, parameter);
                    break;
                //not equal !=
                case AppEnum.WhereOperation.NotEqual:
                    condition = Expression.NotEqual(memberAccess, filter);
                    lambda = Expression.Lambda(condition, parameter);
                    break;
                //string.Contains()
                case AppEnum.WhereOperation.Contains:
