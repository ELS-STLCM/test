...
{
    public class PatientVisitAppointmentDocument : KeyValueRepository<PatientVisit>, IPatientVisitAppointmentDocument
    { 
        #region Appointments- Deserialiazation based on View Types of calendar and Providers

        /// <summary>
        ///  Get all appointments under patient visit type node
        /// </summary>
        /// <param name="providerType"></param>
        /// <param name="urlForAppointments"> </param>
        /// <returns></returns>
        private  List<Appointment> GetAllAppointmentsInPatientVisit(AppEnum.ProviderType providerType,string urlForAppointments)
        {
            List<Appointment> appointmentsInPatientVisit = new List<Appointment>();
            switch (providerType)
            {
                case AppEnum.ProviderType.SingleProvider:
                 Dictionary<string, Dictionary<string, Dictionary<string, Dictionary<string, Dictionary<string,PatientVisit>>>>> patientVisitAppointments=new Dictionary<string, Dictionary<string, Dictionary<string, Dictionary<string, Dictionary<string, PatientVisit>>>>>();
                 foreach (var months in patientVisitAppointments)
                    {
                        foreach (var weeks in months.Value)
                        {
                            foreach (var days in weeks.Value)
                            {
                                foreach (var providers in days.Value)
                                {
                                    ...
                                    {
                                        appointmentsInPatientVisit.Add(appointments.Value);
                                    }
                                }
                            }
                        }
               ...
               Dictionary<string, Dictionary<string, Dictionary<string, Dictionary<string, PatientVisit>>>> multiProviderpatientVisit=new Dictionary<string, Dictionary<string, Dictionary<string, Dictionary<string, PatientVisit>>>>();
               foreach (var months in multiProviderpatientVisit)
                    {
                        foreach (var weeks in months.Value)
                        {
                            foreach (var days in weeks.Value)
            ...
            return appointmentsInPatientVisit;
        }
        ...
        private List<Appointment> GetAllAppointmentsInDay(string urlToGetAppointments, AppEnum.ProviderType providerType, AppEnum.CalendarFilterTypes calendarFilterType)
        {
            ...
            switch (providerType)
            {
                case AppEnum.ProviderType.SingleProvider:
                    ...
                    {
                            Dictionary<string , PatientVisit> appointmentForAproviderInADay= AppCommon.IsValidJsonString(jsonString)
                                                                                                                           ? JsonSerializer.DeserializeObject<Dictionary<string, PatientVisit>>
                                                                                                                                 (jsonString)
                                                                                                                           : new Dictionary<string, PatientVisit>();  
                            foreach (var patientVisit in appointmentForAproviderInADay)
                            {
                                appointments.Add(patientVisit.Value);
                            }
                    }
                    else
                    {
                        Dictionary<string, Dictionary<string, PatientVisit>> appoinmentsForADaySingleProvider =
                            AppCommon.IsValidJsonString(jsonString)
                                ? JsonSerializer.DeserializeObject<Dictionary<string, Dictionary<string, PatientVisit>>>
                                      (jsonString)
                                : new Dictionary<string, Dictionary<string, PatientVisit>>();
                        foreach (var appointmentsDay in appoinmentsForADaySingleProvider)
                        {
                            //Provider Node
                            foreach (var appointment in appointmentsDay.Value) //List of appointments 
                            {
                ...
                case AppEnum.ProviderType.MultiProvider:
                    jsonString = GetJsonDocument(urlToGetAppointments);
                    Dictionary<string, PatientVisit> appoinmentsForADay = AppCommon.IsValidJsonString(jsonString) ? JsonSerializer.DeserializeObject<Dictionary<string, PatientVisit>>(jsonString) : new Dictionary<string, PatientVisit>();
        ...
        private List<Appointment> GetAllAppointmentsInWeek(string urlToGetAppointments, AppEnum.ProviderType providerType, AppEnum.CalendarFilterTypes calendarFilterType, CalendarFilterProxy calendarFilterProxy)
        {
            ...
            switch (providerType)
            {
                case AppEnum.ProviderType.SingleProvider:
                    jsonString = GetJsonDocument(urlToGetAppointments);
                    Dictionary<string, Dictionary<string, Dictionary<string, PatientVisit>>> appoinmentsForAWeekSingleProvider = AppCommon.IsValidJsonString(jsonString) ? JsonSerializer.DeserializeObject<Dictionary<string, Dictionary<string, Dictionary<string, PatientVisit>>>>(jsonString) : new Dictionary<string, Dictionary<string, Dictionary<string, PatientVisit>>>();
                    //days
                ...
                case AppEnum.ProviderType.MultiProvider:
                    jsonString = GetJsonDocument(urlToGetAppointments);
                    Dictionary<string, Dictionary<string, PatientVisit>> appoinmentsForAWeek = AppCommon.IsValidJsonString(jsonString) ? JsonSerializer.DeserializeObject<Dictionary<string, Dictionary<string, PatientVisit>>>(jsonString) : new Dictionary<string, Dictionary<string, PatientVisit>>();
                    //days
        ...
        private List<Appointment> GetAllAppointmentsInAMonth(string urlTogetAppointments, AppEnum.ProviderType providerType, AppEnum.CalendarFilterTypes calendarFilterType,CalendarFilterProxy calendarFilterProxy)
        {
            ...
            switch (providerType)
            {
                case AppEnum.ProviderType.SingleProvider:
                    jsonString = GetJsonDocument(urlTogetAppointments);
                    Dictionary<string, Dictionary<string, Dictionary<string, Dictionary<string, PatientVisit>>>> appointmentsSingleProvider = AppCommon.IsValidJsonString(jsonString) ? JsonSerializer.DeserializeObject<Dictionary<string, Dictionary<string, Dictionary<string, Dictionary<string, PatientVisit>>>>>(jsonString) : new Dictionary<string, Dictionary<string, Dictionary<string, Dictionary<string, PatientVisit>>>>();
                    //Weeks
                ...
                case AppEnum.ProviderType.MultiProvider:
                    jsonString = GetJsonDocument(urlTogetAppointments);
                    Dictionary<string, Dictionary<string, Dictionary<string, PatientVisit>>> appointmentsMultiProvider = AppCommon.IsValidJsonString(jsonString) ? JsonSerializer.DeserializeObject<Dictionary<string, Dictionary<string, Dictionary<string, PatientVisit>>>>(jsonString) : new Dictionary<string, Dictionary<string, Dictionary<string, PatientVisit>>>();
                    //weeks
        ...
        /// <param name="calendarFilterType"></param>
        /// <returns></returns>
        public List<Appointment> GetAppointmentsForPatientVisit(CalendarFilterProxy calendarFilter, AppEnum.CalendarFilterTypes calendarFilterType)
        {
            ...
            if (calendarFilterType == AppEnum.CalendarFilterTypes.Patient)
            {
                urlToGetAppointments =DbCommon.GetUrlBasedNodeTypeForFilter(
                                                                    GetAssignmentUrl(dropBox, DocumentPath.Module.PatientVisitAppointment,
                                                                                               AppEnum.ProviderType.SingleProvider.ToString()), calendarFilter,
                                                                                                calendarFilterType, AppEnum.ProviderType.SingleProvider);

                appointments.AddRange(GetAllAppointmentsInPatientVisit(AppEnum.ProviderType.SingleProvider, urlToGetAppointments));
                urlToGetAppointments = DbCommon.GetUrlBasedNodeTypeForFilter(
                                                    GetAssignmentUrl(dropBox, DocumentPath.Module.PatientVisitAppointment,
                                                                               AppEnum.ProviderType.MultiProvider.ToString()), calendarFilter,
                                                                                calendarFilterType, AppEnum.ProviderType.MultiProvider);
                appointments.AddRange(GetAllAppointmentsInPatientVisit(AppEnum.ProviderType.MultiProvider, urlToGetAppointments));
            }
            else
            {
