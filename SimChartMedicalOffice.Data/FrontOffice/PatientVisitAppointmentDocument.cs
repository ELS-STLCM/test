...
namespace SimChartMedicalOffice.Data.FrontOffice
{
    public class PatientVisitAppointmentDocument : KeyValueRepository<PatientVisit>, IPatientVisitAppointmentDocument
    {
        public override string Url
        {
            get
            {
                //"SimApp/Courses/{CourseId}/{Role}/Assignments/{SID}/Appointments/Type/PatientVisit/{YYYYMM}/{dd}/{ProvideInteger}/{DictionaryofAppointments}
                return _patientVisitIdentifier+"/{5}/{6}/{7}"; 
            }
        }

        //To Form the Url Dynamically to fetch the data for an assignment type;
        private string _patientVisitIdentifier = "SimApp/Courses/{0}/{1}/{2}/Assignments/{3}/Appointments/Type/PatientVisit/{4}";
        ...
        /// <returns></returns>
        private List<Appointment> GetAllAppointmentsInADay(Dictionary<string, Dictionary<string, PatientVisit>> appoinmentsForDays)
        {
        ...
        /// <returns></returns>
        private List<Appointment> GetAllAppointmentsInAMonth(Dictionary<string, Dictionary<string, Dictionary<string, PatientVisit>>> appointmentsInAMonthList, AppEnum.CalendarFilterTypes calendarFilterType, CalendarFilterProxy calendarFilter)
        {
            List<Appointment> appointments = new List<Appointment>();
            //Month Node
        ...
        public List<Appointment> GetAppointmentsForPatientVisit(CalendarFilterProxy calendarFilter, AppEnum.CalendarFilterTypes calendarFilterType)
        {
            ...
            switch (calendarViewTypes)
            {
                case AppEnum.CalendarViewTypes.month:
                    urlsForAppointments.Add(AppCommon.GetUrlForMonthFilter(_patientVisitIdentifier, calendarFilter.CourseId, calendarFilter.Role, calendarFilter.ScenarioId, calendarFilter.StartDate, calendarFilter.UserId));
                    foreach (var urlItem in urlsForAppointments)
                    {
                        string jsonString = GetJsonDocument(urlItem); 
                        //Month/day/provider/appointments
                        Dictionary<string, Dictionary<string, Dictionary<string, PatientVisit>>> appointmentsInAMonthList = AppCommon.isValidJsonString(jsonString) ? JsonSerializer.DeserializeObject<Dictionary<string, Dictionary<string, Dictionary<string, PatientVisit>>>>(jsonString) : new Dictionary<string, Dictionary<string, Dictionary<string, PatientVisit>>>();
                ...
                case AppEnum.CalendarViewTypes.agendaWeek:
                    ...
                    foreach (var item in urlsForAppointments)
                    {
                        ...
                        if (calendarFilterType == AppEnum.CalendarFilterTypes.Provider)
                        {
                            ...
                            Dictionary<string, PatientVisit> appointmentForDays = AppCommon.isValidJsonString(jsonString) ? JsonSerializer.DeserializeObject<Dictionary<string, PatientVisit>>(jsonString) : new Dictionary<string, PatientVisit>();
                        ...
                        else
                        {
                            ...
                            Dictionary<string, Dictionary<string, PatientVisit>> appoinmentsForDays = AppCommon.isValidJsonString(jsonString) ? JsonSerializer.DeserializeObject<Dictionary<string, Dictionary<string, PatientVisit>>>(jsonString) : new Dictionary<string, Dictionary<string, PatientVisit>>();
